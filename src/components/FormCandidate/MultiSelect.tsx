...
import { getProfessionlist } from "@/db/queries/professions"; // Импортируем функцию для получения списка локаций
...
const MultiSelect: React.FC<DropdownProps> = ({ id }) => {
  ...
  useEffect(() => {
    const loadOptions = async ({ query }: { query: string }) => {
      try {
        const profession = await getProfessionlist(query); // Получаем список локаций
        const newOptions: Option[] = profession.map(profession => ({
          value: profession.id,
          text: profession.name || "", // Проверяем наличие имени локации
          selected: false
        }));
    ...
    loadOptions({ query: "" }); // Добавляем пустой запрос
  }, []);
  ...
  return (
    <div className="relative z-50">
      <label className="mb-3 block text-sm font-medium text-black dark:text-white">
        Дополнительные навыки\нет в призма схеме
      </label>
      <div>
                  ...
                  <div className="flex flex-auto flex-wrap gap-3">
                    ...
                    {selected.length === 0 && (
                      <div className="flex-1">
                        <input
                          placeholder="Дополнительные специальности"
